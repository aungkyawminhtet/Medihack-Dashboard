...
import { TransportRequest, RequestPriority, RequestStatus } from '../types/transport';
              ...
              sortedRequests.map(request => (
                <Card
                  ...
                  className={cn(
                    ...
                    selectedRequestId === request.id && 'ring-2 ring-blue-500 shadow-md'
                  )}
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <span className="capitalize">{request.equipmentType}</span>
                      </div>
                    </div>

                    {/* Patient Info */}
                    <div className="mb-3">
                      <div className="font-semibold text-sm">{request.patientInfo.name}</div>
                      <div className="text-xs text-muted-foreground">
                        ID: {request.patientInfo.id} • Room: {request.patientInfo.roomNumber}
                    ...
                    <div className="flex items-center gap-2 text-xs mb-3 bg-gray-50 p-2 rounded">
                      <div className="flex items-center gap-1 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium truncate">{request.origin.zone}</div>
                      ...
                      <div className="flex items-center gap-1 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium truncate">{request.destination.zone}</div>
                          <div className="text-muted-foreground truncate">
                            Floor {request.destination.floor} • {request.destination.room}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {request.notes && (
                      <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded text-xs">
                        <div className="flex items-start gap-1">
                          ...
                          <span className="text-amber-800">{request.notes}</span>
                        </div>
                      </div>
                    )}
